...
In this case, the Normal CI are more rigid and fewer, compared to the Percentile CI edges appear in the difference graph $G^{\Delta}(t)$. So, if someone wants to return only "good edges" it is better to stay stick with the normal CI. On the other hand, if someone wants to return more edges, i.e including not only the strongest one, it is batter to move to the Percentile intervals.

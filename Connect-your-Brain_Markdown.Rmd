...
Since we are using two different approaches we have decided to use two different thresholds. 
...
Below is returned the GASD(t) obtained by following the approach 1.

![](images_asd/Bonferroni/asd_1.png)
...
Below is returned the GTD(t) obtained by following the approach 1.

![](images_td/Bonferroni/td_1.png)
...
Here, since we are following the approach 2, we first compute graphs for each person and then we combine them to return the GASD(T) as a weighted graph.
```{r include=TRUE, message=FALSE, eval=FALSE, class.source="bg-success", warning=FALSE}
...
Below are plotted the first 4 graphs, representing the first 4 people in the given dataset (asd people). Inside the folder "-----" are stored all the 12 images graphs.

![](images_asd/Bonferroni/asd_2_person_1.png) ![](images_asd/Bonferroni/asd_2_person_2.png) ![](images_asd/Bonferroni/asd_2_person_3.png) ![](images_asd/Bonferroni/asd_2_person_4.png)
![](images_asd/Bonferroni/asd_2_person_5.png) ![](images_asd/Bonferroni/asd_2_person_6.png)
...
```
![](images_asd/Bonferroni/asd_2.png)

### <span style="color: purple;"> 2.2.2)  TD Graph Connections </span>

Here, since we are following the approach 2, we first compute graphs for each person and then we combine them to return the GTD(T) as a weighted graph.
```{r include=TRUE, message=FALSE, eval=FALSE, class.source="bg-success", warning=FALSE}
...
Below are plotted the first 4 graphs, representing the first 4 people in the given dataset (td people). Inside the folder "-----" are stored all the 12 images graphs.

![](images_td/Bonferroni/td_2_person_1.png) ![](images_td/Bonferroni/td_2_person_2.png) ![](images_td/Bonferroni/td_2_person_3.png) ![](images_td/Bonferroni/td_2_person_4.png)
![](images_td/Bonferroni/td_2_person_5.png) ![](images_td/Bonferroni/td_2_person_6.png)
...
```
![](images_td/Bonferroni/td_2.png)


# <span style="color: purple;"> 3)  Correlation without Bonferroni </span>

In this section we repeat the same procedures described above (approach 1 and approch2 link) but working with unadjusted intervals (No Bonferroni corrections).

## <span style="color: purple;"> 3.1)  Approach 1 without Bonferroni </span>

Here below is reported the code used to compute the graphs for both asd and td people (following approach 1 link) without applying the Bonferroni correction.
```{r include=TRUE, message=FALSE, eval=FALSE, class.source="bg-success", warning=FALSE}
...
Here is it possible to see the new graphs generated without the Bonferroni correction.
![](images_asd/no_Bonferroni/asd_1_no_bonferroni.png)![](images_td/no_Bonferroni/td_1_no_bonferroni.png)

## <span style="color: purple;"> 3.2)  Approach 2 without Bonferroni </span>

Here below is reported the code used to compute the graphs for both asd and td ( following the Approach 2(link)) people without applying the Bonferroni correction.
```{r include=TRUE, message=FALSE, eval=FALSE, class.source="bg-success", warning=FALSE}
...
Here is it possible to see the new graphs generated without the Bonferroni correction.
![](images_asd/no_Bonferroni/asd_2_no_bonferroni.png)![](images_td/no_Bonferroni/td_2_no_bonferroni.png)

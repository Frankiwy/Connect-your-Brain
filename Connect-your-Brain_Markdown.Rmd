...
We decided to stick with the Pearson-Correlation-Coefficient. In the bootstrap procedure we build __Percentile__ confidence intervals: 

\[quantile(data, \frac{\alpha}{2}, 1-\frac{\alpha}{2}) \] 

for which we assume the existence of symmetrizing transformation condition.
...
where we stored S difference correlation matrices. The reason why we used a tensor tot store the bootstrap results is that this allowed us to get .

The Non-Parametric bootstrap procedure involves a resampling method from the population (where the population is considered the in-our-hand-data) so, in every $S^{th}$ step, both for ASD and TD groups we picked at random 12 people __with__ replacement. Than, by fallowing [Approach 1](#A1), we computed the Pearson-Correlation-Coefficient for both groups (ASD, TD) and after that the Difference-Correlation is computed ($|\Delta_{jk}| = |\rho_{jk}^{ASD} - \rho_{jk}^{TD}|$). However, since the $\Delta^{ASD-TD}$ Difference-Correlation-Matrix $\in [-2;2]$ we have devided it by 2 in order to have it $\in [-1;1]$. 
